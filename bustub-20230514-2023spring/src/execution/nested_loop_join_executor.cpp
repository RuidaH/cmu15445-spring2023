...
// void NestedLoopJoinExecutor::OutputTuple(const Schema &left_table_schema, const Schema &right_table_schema, Tuple
// *left_tuple, Tuple *right_tuple) {
//   std::vector<Value> new_tuple_values;
//   new_tuple_values.reserve(GetOutputSchema().GetColumnCount());  // 提前分配好固定大小的内存空间, 避免动态扩容
//   (很聪明的优化)
